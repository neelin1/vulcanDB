...
12. When defining a 1:n table, its surrogate primary key (e.g., `employer_id`) MUST be described as auto-incrementing or sequentially generated by the database.
13. Generally refrain from renaming columns from the raw data, but if you do, explain why.
14. Don't combine columns from the raw data into a single column in the table. The system cannot handle this.
15. Please pay particular attention to the date columns, sometimes they are formatted in a non-standard way. Since this system is automated, there is no way to edit the raw data. Please follow the format in the raw data sample, paying attention to if its in dd-mm-yyyy or mm-dd-yyyy format or yyyy-mm-dd or something else. So when discussing that column, mention its format, so our constraint generation can take this into account.
"""
    if data.get("single_table", False):
        system_prompt += """
16. The user has indicated that only a single table should be created for this database. Adhere strictly to this requirement. Do not create multiple tables."""
...
12. Please pay attention to the raw data sample. Make sure your constraints align with it.
13. Please pay particular attention to the date columns, sometimes they are formatted in a non-standard way. Since this system is automated, there is no way to edit the raw data. Please follow the format in the raw data sample, paying attention to if its in dd-mm-yyyy or mm-dd-yyyy format or yyyy-mm-dd or something else, and adjusting your constraints accordingly.
"""
    if data.get("table_list") and len(data["table_list"]) > 1:
        system_prompt += """
14. Please ensure that foreign keys reference the surrogate id, not the natural id."""
...
14. (If `Table Traits` are provided and contain relevant entries): For tables identified as '1:n' in `Table Traits` (via `relation_to_raw: \"1:n\"` and the presence of `one_to_n` details):
    ...
    b. The column specified in `one_to_n.surrogate_pk_col` MUST be defined as an auto-incrementing `PRIMARY KEY` (e.g., `SERIAL PRIMARY KEY` or `INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY` for PostgreSQL, or an equivalent mechanism for other databases).
    ...
    if data.get("table_list") and len(data["table_list"]) > 1:
        system_prompt += """
15. Please ensure that foreign keys reference the surrogate id, not the natural id."""
    system_prompt += """
